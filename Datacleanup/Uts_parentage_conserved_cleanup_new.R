...
Uts_parentage_conserved_new1 <- left_join(Uts_conserved_parents_new, Uts_Birthyear.info, by = c("id" = "ID")) %>%
  rename(ID = id) %>%
  relocate(c(10:17), .after = ID)

#join data for dams
Uts_parentage_conserved_new_dams <- left_join(Uts_parentage_conserved_new1, Uts_Birthyear.info, by = c("dam" = "ID")) %>%
...
#join data for sires
Uts_parentage_conserved_new_dams_sires <- left_join(Uts_parentage_conserved_new_dams, Uts_Birthyear.info, by = c("sire" = "ID")) %>%
...
Uts_parentage_conserved_new_all <- Uts_parentage_conserved_new_dams_sires %>%

...
#rename database
Uts_parentage_cons <- Uts_parentage_conserved.21.06.18
...
Uts_cons_dams_wide_rmNA <- Uts_cons_dams_wide %>%
  filter(dam != "NA")%>%
  replace(is.na(.), 0)

#sum individuals across rows
Uts_cons_dams_total <- Uts_cons_dams_wide_rmNA %>%
  rowwise() %>% 
  mutate(n.offspring = rowSums(across(c("0+", "1+", "2-3+", "Adult")))) %>%
  rename(cohort.year = birthyear.int.off)

#check total offspring
Uts_cons_dams_total %>% ungroup() %>% tally(n.offspring)

#join datasets
Uts_cons_dams_all <- left_join(Uts_cons_dams_total, Uts_Birthyear_Calc_info, by = c("dam" = "ID")) %>%
  relocate(c(8:16), .after = dam)

#find minimum cohort year
Uts_cons_dams_cohort.min <- Uts_cons_dams_all %>%
  group_by(dam) %>%
...
Uts_cons_dams_min.combine <- left_join(Uts_cons_dams_all, Uts_cons_dams_cohort.min, by = "dam")

#check, how many females had cohort that was before collection?
Uts_cons_dams_min.combine.check <- Uts_cons_dams_min.combine %>%
  rowwise() %>% 
  mutate(respawn.check = ifelse(Respawner == 0 & firstcohort <= year, 1, 
                                ifelse(Respawner == 1 & firstcohort <= year-2, 1, 0)))

#reset firstcohort for Uts_cons_dams_min.combine for individuals that had a few offspring in an earlier chort year
Uts_cons_dams_min.combine.fix <- Uts_cons_dams_min.combine.check %>%
...
Uts_cons_dams_fwage <- Uts_cons_dams_min.combine.fix %>%
  mutate(Scale.smoltage = replace_na(Scale.smoltage, 4)) 
  
#caculate age at maturity
Uts_cons_dam_cohort_AAM <- Uts_cons_dams_fwage %>%  
  group_by(dam) %>%
...
Uts_cons_dams_cohort_assignment <- Uts_cons_dam_cohort_AAM %>%
  ungroup() %>%
  rowwise(dam) %>%
...
Uts_cons_dams_cohort_year_rank <-Uts_cons_dams_cohort_assignment %>%
 group_by(dam, cohort) %>%
...
Uts_cons_dams_cohort_year_rank %>% group_by(dam, cohort) %>% filter(cohort == "1") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_dams_cohort_year_rank %>% group_by(dam, cohort) %>% filter(cohort == "2") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_dams_cohort_year_rank %>% group_by(dam, cohort) %>% filter(cohort == "3") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_dams_cohort_year_rank %>% group_by(dam, cohort) %>% filter(cohort == "4") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_dams_cohort_year_rank %>% group_by(dam, cohort) %>% filter(cohort == "5") %>% count(cohort.year.rank) %>% filter(n > 1)
#fix issue with Uts_14A_024
Uts_cons_dams_cohort_year_fix <- Uts_cons_dams_cohort_year_rank %>%
...
Uts_cons_dams_cohort_year <- Uts_cons_dams_cohort_year_fix %>%
...
Uts_cons_dams_cohort_year_fill <-Uts_cons_dams_cohort_year %>% 
  group_by(dam, cohort) %>%
...
Uts_cons_dams_cohort_year_fix <- Uts_cons_dams_cohort_year_fill %>%
...
Uts_cons_dams_cohort.min.fix <- Uts_cons_dams_cohort_year_fix %>%
  group_by(dam) %>%
...
Uts_cons_dams_cohort.min.fix %>% ungroup %>% filter(cohort == "1", cohort.year.rank == "1") %>% count(firstcohort == cohort.year)

#combine for cohorts
dam_cons_cohort_year_combine <- Uts_cons_dams_cohort.min.fix %>%
  group_by(dam, cohort, cohort.year.all) %>%
  summarise(across(11:15, sum))

#check distribution
dam_cons_cohort_year_combine %>% ungroup() %>% summarise(across(4:8, sum))

#select row info from Uts_cons_dams_cohort_year
dams_cons_cohort_AAM_info <- Uts_cons_dam_cohort_AAM %>%
...
#select distinct row info from Uts_cons_dams_cohort_AAM
dams_cons_cohort_AAM_distinct <- Uts_cons_dam_cohort_AAM %>%
...
#join datasets
dam_cons_cohort_total <- left_join(dams_cons_cohort_AAM_distinct, dam_cons_cohort_year_combine, by = "dam")

#tally cohorts
dam_cons_cohort_total_count <- dam_cons_cohort_total %>%
  group_by(dam) %>%
  count() %>%
  rename(cohort.total = n) 

#designate semelparous and iteroparous
dam_cons_cohort_total_RESP <- dam_cons_cohort_total_count %>%
  mutate(respawner.gen = ifelse(cohort.total > 1, "iteroparous", "semelparous")) %>%
  ungroup()

#combine sire and cohort counts
dam_cons_cohort_all <- left_join(dam_cons_cohort_total, dam_cons_cohort_total_RESP, "dam")

#find minimum cohort #
dam_cons_cohort_min.cohort <- dam_cons_cohort_all %>%
  group_by(dam) %>%
  mutate(min.cohort = min(cohort))

#rank cohorts
dam_cons_cohort_total_rank <- dam_cons_cohort_min.cohort %>%
  group_by(dam) %>%
  mutate(cohort.rank = ifelse(min.cohort == 1, dense_rank(cohort),
                              ifelse(min.cohort == 2, dense_rank(cohort)+1, NA)))

#check total offspring
dam_cons_cohort_total_rank %>% ungroup() %>% tally(n.offspring)
...
#bring in SNP data
dam_cons_cohort_SNP <- left_join(dam_cons_cohort_total_rank, UtsSNP_VIP, c("dam" = "ID"))


####working with sires####
#summarize (count) all offspring per sire 
Uts_cons_sires <- Uts_parentage_cons %>%
  group_by(sire, class.cor.off, birthyear.int.off) %>%
  summarise_at(c("type.off"), funs(n.off = sum(!is.na(.))))

#pivot wider
Uts_cons_sires_wide <- Uts_cons_sires %>%
  pivot_wider(
    names_from = c(class.cor.off),
    names_sep = "_",
    values_from = n.off)
...
Uts_cons_sires_wide_rmNA <- Uts_cons_sires_wide %>%
  filter(sire != "NA")%>%
  replace(is.na(.), 0)

#sum individuals across rows
Uts_cons_sires_total <- Uts_cons_sires_wide_rmNA %>%
  rowwise() %>% 
  mutate(n.offspring = rowSums(across(c("0+", "1+", "2-3+", "Adult")))) %>%
  rename(cohort.year = birthyear.int.off)

#join datasets
Uts_cons_sires_all <- left_join(Uts_cons_sires_total, Uts_Birthyear_Calc_info, by = c("sire" = "ID")) %>%
  relocate(c(8:16), .after = sire) 

#combine into cohorts and assign a cohort number
Uts_cons_sires_cohort.min <- Uts_cons_sires_all %>%
  group_by(sire) %>%
  summarize(firstcohort = ifelse(type == "Adult" & Respawner == "0", min(cohort.year),
                                 ifelse(type == "Adult" & Respawner == "1", min(cohort.year), 
                                        ifelse(type == "Offspring", min(cohort.year), NA)))) %>%
...
#combine minimum cohort with Uts_conssires_MMP, fix respawner info 
Uts_cons_sires_min.combine <- left_join(Uts_cons_sires_all, Uts_cons_sires_cohort.min, "sire")

#check, how many females had cohort that was before collection?
Uts_cons_sires_min.combine.check <- Uts_cons_sires_min.combine %>%
  rowwise() %>% 
  mutate(respawn.check = ifelse(Respawner == 0 & firstcohort <= year, 1, 
                                ifelse(Respawner == 1 & firstcohort <= year-2, 1, 0)))

#set respawner.info = NA for individuals that aren't respawners
Uts_cons_sires_min.combine.fix <- Uts_cons_sires_min.combine.check %>%
  mutate(respawner.info = ifelse(respawner.info == "1S1", respawner.info, NA)) 

#reset firstcohort for Uts_cons_sires_min.combine for individuals that had a few offspring in an earlier chort year
#issue with Uts_11A_17, seaage = 3, no information on repeat spawning from scale, first cohort in 2010, consistent with 1S1 repeat spawner, 1 adult offspring
#issue with Uts_11A_40, seaage = 4, no information on repeat spawning from scale info, first cohort in 2007, consistent with 0S1 (mmp) or 1S1 if offspring aged incorrectly, 1 adult offspring
#issue with Uts_13A_014, seaage = 1, no information on repeat spawning from scale, first cohort in 2012, consistent with 0S1 (mmp) or 1S1 if offspring aged incorrectly, 1 1+ offspring
#issue with Uts_13A_024, seaage = 3, no information on repeat spawning from scale data, first  cohort 2012, consistent with 1S1, many offspring 
#issue with Uts_13A_043, seaage = 3, no information on repeat spawning from scale data, first cohort 2012, consistent with 1S1, many offspring
#issue with Uts_16A_041, seaage = 2, no information on repeat spawning from scale data, first cohort 2015, consistent with 0S1 (mmp), 2 offspring
#issue with Uts_17_071, seaage = 3, no information on repeat spawning from scale data, first cohort 2014, consistent with 0S1 (mmp), 1 offspring
Uts_cons_sires_min.combine.fix2 <- Uts_cons_sires_min.combine.fix %>%
...
#reset firstcohort for Uts_cons_dams_min.combine for individuals that had a few offspring in an earlier cohort year                                                      
Uts_cons_sires_min.combine.fix3 <- Uts_cons_sires_min.combine.fix2 %>%
...
Uts_cons_sires_min.combine.fix4 <- Uts_cons_sires_min.combine.fix3 %>%
...
#add fw age (smolt.scale.age) for individuals missing age
Uts_cons_sires_fwage <- Uts_cons_sires_min.combine.fix4 %>%
  mutate(Scale.smoltage = replace_na(Scale.smoltage, 4)) 

#calculate seaage at maturity for first cohort
Uts_cons_sire_cohort_AAM <- Uts_cons_sires_fwage %>%  
  group_by(sire) %>%
  mutate(seaageatmaturity = ifelse(class.cor == "0+", firstcohort-birthyear.int-Scale.smoltage,
                                   ifelse(class.cor == "1+", firstcohort-birthyear.int-Scale.smoltage,    
                                          ifelse(class.cor == "2-3+", firstcohort-birthyear.int-Scale.smoltage,
...
Uts_cons_sires_cohort_assignment <- Uts_cons_sire_cohort_AAM %>%
  rowwise() %>%
  mutate(cohort = ifelse(cohort.year == firstcohort-1, 1, 
                         ifelse(cohort.year == firstcohort, 1,   
                                ifelse(cohort.year == firstcohort +1, 1, 
...
Uts_cons_sire_cohort_year_rank <-Uts_cons_sires_cohort_assignment %>%
...
Uts_cons_sire_cohort_year_rank %>% group_by(sire, cohort) %>% filter(cohort == "1") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_sire_cohort_year_rank %>% group_by(sire, cohort) %>% filter(cohort == "2") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_sire_cohort_year_rank %>% group_by(sire, cohort) %>% filter(cohort == "3") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_sire_cohort_year_rank %>% group_by(sire, cohort) %>% filter(cohort == "4") %>% count(cohort.year.rank) %>% filter(n > 1)
Uts_cons_sire_cohort_year_rank %>% group_by(sire, cohort) %>% filter(cohort == "5") %>% count(cohort.year.rank) %>% filter(n > 1)

#estimate the 2nd, 3rd, 4th and 5th cohort year
Uts_cons_sire_cohort_year <- Uts_cons_sire_cohort_year_rank %>%
  mutate(cohort.year.all = ifelse(cohort == "1" & cohort.year.rank == "1", cohort.year, 
                                  ifelse(cohort == "2" & cohort.year.rank == "1", cohort.year, 
                                         ifelse(cohort == "3" & cohort.year.rank == "1", cohort.year, 
...
#fill down cohort.year
Uts_cons_sire_cohort_year_fill <-Uts_cons_sire_cohort_year %>% 
  group_by(sire, cohort) %>%
  fill(cohort.year.all) 
...
Uts_cons_sire_cohort_year_fix <- Uts_cons_sire_cohort_year_fill %>%
...
Uts_cons_sire_cohort_year_fix2 <- Uts_cons_sire_cohort_year_fix %>%
  ...
  #reset first cohort based on ranked 1st cohort
Uts_cons_sire_cohort.min.fix <- Uts_cons_sire_cohort_year_fix2 %>%
  group_by(sire) %>%
  mutate(firstcohort = ifelse(cohort == "1" & cohort.year.rank == "1", cohort.year.all, NA)) %>%
  fill(firstcohort) %>%
...
sire_cons_cohort_year_combine <- Uts_cons_sire_cohort.min.fix %>%
  group_by(sire, cohort, cohort.year.all) %>%
  summarise(across(11:15, sum))

#calculate mean % of classes /sire/cohort
sire_cons_cohort_year_combine %>% ungroup() %>% summarise(across(4:8, sum))

#select row info from Uts_cons_sire_cohort_AAM
sire_cons_cohort_AAM_info <- Uts_cons_sire_cohort_AAM %>%
...
#select distinct row info from Uts_cons_sire_cohort_AAM
sire_cons_cohort_AAM_distinct <- Uts_cons_sire_cohort_AAM %>%
  ungroup() %>%
...
#join datasets
sire_cons_cohort_total <- left_join(sire_cons_cohort_AAM_distinct, sire_cons_cohort_year_combine, by = "sire")

#check total offspring
sire_cons_cohort_total %>% ungroup() %>% tally(n.offspring)

#tally cohorts
sire_cons_cohort_total_count <- sire_cons_cohort_total %>%
  group_by(sire) %>%
  count() %>%
  rename(cohort.total = n) 

#designate semelparous and iteroparous
sire_cons_cohort_total_RESP <- sire_cons_cohort_total_count %>%
  mutate(respawner.gen = ifelse(cohort.total > 1, "iteroparous", "semelparous")) %>%
  ungroup()

#combine sire and cohort counts
sire_cons_cohort_all <- left_join(sire_cons_cohort_total, sire_cons_cohort_total_RESP, "sire")

#check total offspring
sire_cons_cohort_all %>% ungroup() %>% tally(n.offspring)

#find minimum cohort #
sire_cons_cohort_min.cohort <- sire_cons_cohort_all %>%
  group_by(sire) %>%
  mutate(min.cohort = min(cohort))

#rank cohorts
sire_cons_cohort_total_rank <- sire_cons_cohort_min.cohort %>%
  group_by(sire) %>%
  mutate(cohort.rank = ifelse(min.cohort == 1, dense_rank(cohort),
                              ifelse(min.cohort == 2, dense_rank(cohort)+1, NA)))

#check total offspring
sire_cons_cohort_total_rank %>% ungroup() %>% tally(n.offspring)

#calculate whether a mature male parr/adult assuming that 3 years is the maximum year for MMP before maturing as an adult
Uts_cons_sires_MMP <- sire_cons_cohort_total_rank %>%
  rowwise() %>%
...
Uts_cons_mature <- Uts_cons_sires_MMP %>%
  ungroup() %>%
  rowwise(sire) %>% 
...
#bring in SNP data
sire_cons_cohort_SNP <- left_join(Uts_cons_sires_MMP, UtsSNP_VIP, c("sire" = "ID"))

####comparison of males and females####
#rename sire
sire_cons_cohort_SNP_ID <-sire_cons_cohort_SNP %>%
  rename(ID = sire) %>%
  mutate(sex = "sire") %>%
  relocate(sex, .after = ID) 

#rename dam
dam_cons_cohort_SNP_ID <-dam_cons_cohort_SNP %>%
  rename(ID = dam) %>%
  mutate(sex = "dam") %>%
  relocate(sex, .after = ID) 

#join data for males and females
Uts_cons_cohort_SNP_cons_1 <-bind_rows(sire_cons_cohort_SNP_ID, dam_cons_cohort_SNP_ID)  

#caculate cohort max
Uts_cons_cohort_SNP_cons_2 <- Uts_cons_cohort_SNP_cons_1 %>%
group_by(ID) %>%
  mutate(cohort.max = max(cohort.total)) %>%
  mutate(cohort.max.year = max(cohort.year.all)) %>%
  ungroup() 

write.csv(Uts_cons_cohort_SNP_cons_2, "C:/Users/kmo107/OneDrive - UiT Office 365/Projects/Atlantic salmon - Teno River Pedigree/2020 - Utsjoki pedigree data/datacleanup/Uts_cohort_SNP_cons_11.02.22.csv")

#dams
#check which are not correctly assigned
Uts_cons_dams_cohortyear_assignment_check <- Uts_cons_dams_cohort_year_fix %>%
...
#sires
#check which are not correctly assigned
Uts_cons_sires_cohortyear_assignment_check <- Uts_cons_sire_cohort_year_fix %>%
  mutate(cohort.year.correct = if_else(cohort.year == cohort.year.all, n.offspring, 0)) %>%
  mutate(cohort.year.incorrect = if_else(cohort.year != cohort.year.all, n.offspring, 0))
